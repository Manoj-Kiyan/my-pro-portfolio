  ...
  return (
    <>
      ...
      <nav className="group hidden md:flex fixed left-0 top-0 z-50 h-screen w-20 flex-col justify-between overflow-hidden
        ...
        <div data-lenis-prevent="true" className="flex flex-col h-full justify-between overflow-y-auto w-full relative z-10 pb-4 [&::-webkit-scrollbar]:hidden" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
          
          {/* TOP SECTION */}
          <div className="flex flex-col w-[260px] px-4 gap-6">
            
            {/* Logo */}
            <div className="flex items-center whitespace-nowrap px-2 mb-2 mt-4">
            ...
            <button 
              onClick={openSearch}
              className="group/btn relative flex items-center whitespace-nowrap rounded-xl px-3 py-3 w-[220px] text-neutral-400 transition-all hover:text-white hover:bg-white/5 overflow-hidden shrink-0"
            >
            ...
            {/* Navigation Links */}
            <div className="flex w-full flex-col gap-2 mt-2 shrink-0">
              {navLinks.map((link) => (
                <Link 
                  key={link.name} 
                  href={link.href}
                  onClick={(e) => handleLinkClick(e, link.href)}
                  className="group/link relative flex items-center whitespace-nowrap rounded-xl px-3 py-3 w-[220px] text-neutral-400 transition-all hover:text-white hover:bg-white/5 overflow-hidden shrink-0"
                >
            ...
            {/* 🔴 NEW: API KEY CONFIGURATION IN NAVBAR */}
            <div className="w-[220px] mt-4 mb-4 relative z-20 group/config shrink-0">
               {/* The Trigger Button */}
               <button 
                 className="w-full flex items-center justify-between px-3 py-3 rounded-xl border border-white/5 bg-black/40 text-neutral-400 hover:text-white hover:bg-white/10 hover:border-purple-500/30 transition-all overflow-hidden"
                 onClick={(e) => {
               ...
               >
                  <div className="flex items-center gap-3">
                     ...
                     <span className="font-mono text-[10px] tracking-widest font-semibold opacity-0 transition-all duration-500 translate-x-[-10px] group-hover/config:translate-x-0 group-hover/config:opacity-100">LLM_CONFIG</span>
                  </div>
                  <svg className="w-4 h-4 opacity-0 transition-all duration-500 group-hover/config:opacity-100" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="6 9 12 15 18 9"></polyline></svg>
               </button>

               {/* The Expandable Panel */}
               <div id="api-config-panel" className="hidden mt-2 p-3 rounded-xl border border-purple-500/30 bg-neutral-900/90 backdrop-blur-xl shadow-[0_0_30px_rgba(107,33,168,0.3)] animate-in slide-in-from-top-2 absolute left-0 w-[240px]">
                  <div className="mb-3">
                    <label className="block font-mono text-[9px] text-purple-400 mb-1 uppercase tracking-widest">Select Core</label>
                    <select 
                      id="nav-model-select"
                      className="w-full bg-black/50 border border-white/10 rounded-lg px-2 py-1.5 text-xs text-white font-sans focus:outline-none focus:border-purple-500/50 appearance-none cursor-pointer"
                      defaultValue="Gemini"
                    >
                  ...
                  <div>
                    <label className="block font-mono text-[9px] text-purple-400 mb-1 uppercase tracking-widest">Access Key</label>
                    <input 
                      type="password" 
                      id="nav-api-key"
                      placeholder="Enter API Key..."
                      className="w-full bg-black/50 border border-white/10 rounded-lg px-2 py-1.5 text-xs text-white font-mono focus:outline-none focus:border-purple-500/50 placeholder-neutral-600 mb-3"
                      onChange={(e) => {
                         localStorage.setItem('mk_api_key', e.target.value);
                      }}
                    />
                  </div>
                  <button 
                    onClick={() => {
                      const model = (document.getElementById('nav-model-select') as HTMLSelectElement)?.value;
                      const key = (document.getElementById('nav-api-key') as HTMLInputElement)?.value.trim();
                    ...
                    className="w-full bg-purple-600 hover:bg-purple-500 text-white font-mono text-[10px] tracking-widest py-1.5 rounded-lg transition-colors border border-purple-400/50 shadow-[0_0_10px_rgba(168,85,247,0.4)]"
                  >
                    SAVE_CONFIG
                  </button>
                  
                  {configStatus.msg && (
                    <div className={`mt-2 text-center text-[9px] font-mono tracking-widest py-1 rounded border animate-in fade-in zoom-in duration-200 ${configStatus.type === 'success' ? 'text-green-400 border-green-500/50 bg-green-500/10 shadow-[0_0_10px_rgba(34,197,94,0.2)]' : 'text-red-400 border-red-500/50 bg-red-500/10 shadow-[0_0_10px_rgba(239,68,68,0.2)]'}`}>
                      {configStatus.msg}
                    </div>
                  )}
               </div>
